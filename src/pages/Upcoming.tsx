...
const Upcoming: React.FC = () => {
   ...
   // compute "next" week based on today + 7 days
   ...
   const iso = useMemo(() => getISOWeekInfo(nextWeekDate), [nextWeekDate]);
   ...
   useEffect(() => {
      // compute ISO week range (start/end as ISO strings) for next week
      const { weekStart, weekEnd } = getISOWeekInfo(nextWeekDate);
   ...
   return (
      <main className="p-6 space-y-6 min-h-[calc(100vh-64px)]">
         <div className="flex items-start justify-between gap-4">
            <div>
               <h1 className="text-2xl font-semibold text-slate-900">Upcoming — {weekId}</h1>
         ...
         <section>
            {loading && <div className="text-sm text-slate-500">Loading upcoming schedule…</div>}
            ...
            {!loading && !error && !schedule && (
               <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                  <h2 className="text-lg font-semibold text-slate-900 mb-2">No schedule for next week</h2>
                  <p className="text-sm text-slate-500 mb-4">
                     There is no saved schedule for next week ({weekId}). You can generate a schedule or manage candidate jobs for this week.
                  </p>
                  ...
                  <div className="flex gap-3">
                     ...
                     <button
                        onClick={async () => {
                           ...
                           try {
                              ...
                              console.error(err);
                              window.alert("Failed to generate schedule for next week.");
                           }
                           setGenerating(false)
                        }}
                     ...
                     >
                        ...
                        {generating ? "Generating…" : "Generate schedule for next week"}
                     </button>
                  </div>
               </div>
            )}
