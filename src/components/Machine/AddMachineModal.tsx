 ...
 * Updated to use the custom Combobox component so option panels render aligned and styled consistently across browsers (fixes Safari dropdown alignment).
 */
const AddMachineModal: React.FC<Props> = ({ open, onClose, machineTypes, statusOptions, onCreate }) => {
   ...
   const typeOptions = machineTypes.map((t) => ({ value: t.uuid, label: t.name }));
   const statusOptionsList = statusOptions.map((s) => ({ value: s.uuid, label: s.name }));
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div>
                     ...
                     <Combobox
                        id="machine-type"
                        options={typeOptions}
                        value={typeUuid || undefined}
                        onChange={(v) => setTypeUuid(v)}
                  ...
                  <div>
                     ...
                     <Combobox
                        ...
                        value={statusUuid || undefined}
                        onChange={(v) => setStatusUuid(v)}
